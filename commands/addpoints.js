/*
    Command Name: addpoints.js
    Function: To Give a User Points
    Clearance: mod+
	Default Enabled: Yes
...
const command = {
    adminOnly: false,
    bigDescription: ("This command allows an administrator or mod to add points to a user's total point count.\n"
        + "Use:\n\t"
        + `!addPoints {user} {int}\n`
        + "Returns:\n\t"
        + "This command returns nothing."),
    description: "Allows an admin or mod the ability to increase someone's points.",
    enabled: true,
    fullName: "Add Points",
    name: "addpoints",
    permissionLevel: "mod"
...
module.exports.run = async (bot, message, args, sql) => {
    ...
    if (!amount) { // If No Amount Detected...
        message.channel.send("You must indicate an amount to increase their points by");
        return react(message, false);
    }
